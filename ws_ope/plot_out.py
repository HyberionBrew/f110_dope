...
def plot_bars(data, target=None, gamma=None):
    ...
    ax.legend(loc='upper left', bbox_to_anchor=(1, 1))
    plt.tight_layout()
    plt.savefig(f"plots/{target}/{gamma}/bar_plot.png")




def plot_discount(gamma=0.85,date="926", path = "/mnt/hdd2/fabian/f1tenth_dope/ws_ope/f1tenth_orl_dataset/data/trajectories.zarr",min_trajectory_length=600, date_2=None,target=None):
    ...
    # importlib.reload(pu)
    logdir = f"logdir/f110_rl_{str(gamma)}_dr_{date}/"
    logdir_mb = f"logdir/f110_rl_{str(gamma)}_mb_{date}/"
    if date_2 is not None:
        logdir_mb = f"logdir/f110_rl_{str(gamma)}_mb_{date_2}/"
    values = pu.plot_everything(logdir,
                    precomputed_returns,
                    precomputed_stds,
                    dataset,
                    logdir_mb=logdir_mb,
                    plt_path = f"plots/{target}/{gamma}",
                    gamma=gamma)
    plt.show()
    return values
...
# first compute some general statistics on the dataset
if False:
    dataset_og = pu.get_data(
                skip_inital=0, 
                # split_trajectories=100,
                alternate_reward=True, 
                # remove_short_trajectories=True,
                ...
                # without_agents=["raceline", "min_lida"]
                path ="/mnt/hdd2/fabian/f1tenth_dope/ws_ope/f1tenth_orl_dataset/data/trajectories_raceline.zarr"
                )
    # below just gives us the dataset we are interested in, accordingly preprocessed
    dataset = pu.get_data(
                skip_inital=0, 
                # split_trajectories=100,
                alternate_reward=True, 
    ...
    pu.plot_dataset_statistics(dataset, path="plots")

    # collisions STILL in the dataset before pruning
    print("Collisions in the dataset before pruning: ")
    pu.count_collision(dataset_og)
    # collisions STILL Present in the dataset after pruning
    print("Collisions in the dataset after pruning: ")
    pu.count_collision(dataset)
...
logdirs = [["922" , "925"], # dr, mb trajectories_min_act
           "926", # trajectories_td_prog
           "926_test", # raceline
           ]
...
for i, target in enumerate(target_datasets):
    for gamma in gammas:
        ...
        if isinstance(logdir, list):
            values = plot_discount(gamma=gamma, date=logdir[0], date_2=logdir[1], path = f"/mnt/hdd2/fabian/f1tenth_dope/ws_ope/f1tenth_orl_dataset/data/{target}",target=target[:-5])
        else:
            values = plot_discount(gamma=gamma, date=logdir, path = f"/mnt/hdd2/fabian/f1tenth_dope/ws_ope/f1tenth_orl_dataset/data/{target}",target=target[:-5])
        
        print(values)
        plot_bars(values, target=target[:-5], gamma=gamma)
